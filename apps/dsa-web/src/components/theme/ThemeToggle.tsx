  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className={cn(
          isNavVariant
            ? 'flex h-7 w-full select-none items-center gap-1.5 rounded-md border border-border/60 bg-card/60 px-2.5 text-[12px] text-muted-foreground transition-colors hover:bg-hover hover:text-foreground'
        ...
        aria-label="切换主题"
      >
        <TriggerIcon className={cn('shrink-0', isNavVariant ? 'h-3.5 w-3.5' : 'h-4 w-4')} />
        {isNavVariant ? (
