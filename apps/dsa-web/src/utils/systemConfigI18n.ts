...
const categoryDescriptionMap: Partial<Record<SystemConfigCategory, string>> = {
  ...
  debate: '配置股票多智能体辩论的启用状态、模型分工、并发限制和通知行为。',
...
const fieldTitleMap: Record<string, string> = {
  ...
  ENABLE_DEBATE_ANALYSIS: '启用多智能体辩论',
  DEBATE_AGENT_MODEL: '辩论模型',
  DEBATE_JUDGE_MODEL: '裁判模型',
  DEBATE_AGENT_PARALLELISM: 'Agent 并行数',
  DEBATE_TASK_CONCURRENCY: '辩论队列并发',
  DEBATE_BATCH_MAX_SIZE: '批量提交上限',
  DEBATE_TIMEOUT_SECONDS: '单股辩论超时（秒）',
  MAX_WORKERS: '最大并发线程数',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  TRADING_DAY_CHECK_ENABLED: '非交易日自动跳过分析；如需强制执行，可关闭此项或使用 --force-run。',
  ENABLE_DEBATE_ANALYSIS: '开启后可提交股票多智能体辩论任务；关闭后辩论接口会返回明确错误，不影响普通分析。默认开启。',
  DEBATE_AGENT_MODEL: 'Phase 1 六个 Agent 与 Phase 2/3 多空辩手使用的模型。留空继承主模型 LITELLM_MODEL；可填写任意已配置 LiteLLM 模型或渠道别名。',
  DEBATE_JUDGE_MODEL: 'Phase 4 裁判使用的模型。留空继承辩论模型；若辩论模型也为空，则继承主模型 LITELLM_MODEL。',
  DEBATE_AGENT_PARALLELISM: '单只股票辩论中 Phase 1 Agent 的最大并行调用数。调低可减少模型限流风险，默认 3。',
  DEBATE_TASK_CONCURRENCY: '完整辩论任务的执行并发。第一阶段固定为 1，多只股票会排队串行执行。',
  DEBATE_BATCH_MAX_SIZE: '单次辩论请求最多接受的股票数量。任务仍会按辩论队列串行执行，默认 10。',
  DEBATE_TIMEOUT_SECONDS: '单只股票完整多智能体辩论的超时时间。超时后任务失败并保留已有日志，默认 300 秒。',
