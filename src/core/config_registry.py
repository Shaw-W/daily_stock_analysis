...
_CATEGORY_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        "category": "agent",
        "title": "Agent 设置",
        "description": "Agent 模式、策略与多 Agent 编排配置。",
        "display_order": 55,
    },
    {
        "category": "backtest",
...
_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "TRADING_DAY_CHECK_ENABLED": {
        "title": "交易日检查",
        "description": "非交易日自动跳过分析；如需强制执行，可关闭此项或使用 --force-run。",
        "category": "system",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "ENABLE_DEBATE_ANALYSIS": {
        "title": "启用多智能体辩论",
        "description": "开启后可提交股票多智能体辩论任务；关闭后 POST /api/v1/analysis/debate 会返回明确错误，不影响普通分析。默认开启。",
        "category": "debate",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "DEBATE_AGENT_MODEL": {
        "title": "辩论模型",
        "description": "Phase 1 六个 Agent 以及 Phase 2/3 多空辩手使用的模型。留空时继承主模型 LITELLM_MODEL；可填写任意已配置 LiteLLM 模型或渠道别名。",
        "category": "debate",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        ...
        "validation": {},
        "display_order": 2,
        "examples": ["DEBATE_AGENT_MODEL=deepseek/deepseek-chat"],
    },
    "DEBATE_JUDGE_MODEL": {
        "title": "裁判模型",
        "description": "Phase 4 裁判使用的模型。留空时继承辩论模型；如果辩论模型也为空，则继承主模型 LITELLM_MODEL。可选示例：openai/gpt-4.1。",
        "category": "debate",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        ...
        "validation": {},
        "display_order": 3,
        "examples": ["DEBATE_JUDGE_MODEL=openai/gpt-4.1"],
    },
    "DEBATE_AGENT_PARALLELISM": {
        "title": "Agent 并行数",
        "description": "单只股票辩论中 Phase 1 Agent 的最大并行调用数。调低可减少模型限流风险；默认 3。",
        "category": "debate",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DEBATE_TASK_CONCURRENCY": {
        "title": "辩论队列并发",
        "description": "完整辩论任务的执行并发。第一阶段固定为 1，多只股票会排队串行执行，避免一次触发过多 LLM 调用。",
        "category": "debate",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DEBATE_BATCH_MAX_SIZE": {
        "title": "批量提交上限",
        "description": "单次辩论请求最多接受的股票数量。任务仍会按辩论队列串行执行；默认 10。",
        "category": "debate",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DEBATE_TIMEOUT_SECONDS": {
        "title": "单股辩论超时（秒）",
        "description": "单只股票完整多智能体辩论的超时时间。超时后任务失败并保留已有日志；默认 300 秒。",
        "category": "debate",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
...
def _infer_category(key: str) -> str:
    ...
    if key.startswith((
        ...
        return "notification"
    if key.startswith("DEBATE_") or key == "ENABLE_DEBATE_ANALYSIS":
        return "debate"
